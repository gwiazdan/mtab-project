...
export const Authors: React.FC = () => {
  ...
  const fetchAuthors = async () => {
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to load authors';
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  const toggleSelectAll = (pageItems: Author[]) => {
  ...
  const handleAddAuthor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to add author');
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to add author';
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || editingAuthorId === null) {
      setError('Please fill in required fields');
      return;
    }
  ...
  const confirmDelete = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete authors');
      setShowDeleteConfirm(false);
    }
  };
  ...
  if (loading) {
    return <div className="text-center py-8 text-gray-400">Loading authors...</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage your authors</p>
        </div>
        <button
      ...
      {selectedIds.size > 0 && (
        <div className="bg-neutral-800 border border-gray-700 p-4 rounded-lg flex items-center justify-between">
          <span className="text-white font-medium">{selectedIds.size} author(s) selected</span>
          ...
          <thead className="bg-neutral-900 border-b border-gray-700">
            <tr>
              <th className="px-6 py-4 text-left">
                <input
                  type="checkbox"
                  checked={selectedIds.size > 0 && paginatedAuthors.every(item => selectedIds.has(item.id))}
                  onChange={() => toggleSelectAll(paginatedAuthors)}
            ...
            {paginatedAuthors.length === 0 ? (
              ...
              paginatedAuthors.map((author) => (
                <React.Fragment key={author.id}>
                  <tr className="hover:bg-neutral-700 transition-colors cursor-pointer" onClick={() => toggleExpanded(author.id)}>
                    ...
                    <tr className="bg-neutral-700">
                      <td colSpan={2} className="px-6 py-4">
                        <div className="space-y-3">
                          {author.bio && (
                            <div>
                              <p className="text-gray-300 text-sm font-medium mb-1">Bio:</p>
      ...
      {showEditForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-neutral-900 border border-gray-700 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh]">
            <div className="p-6 space-y-4">
              <h2 className="text-2xl font-bold text-white">Edit Author</h2>
          ...
          <div className="bg-neutral-900 border border-gray-700 rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="p-6 space-y-4">
              ...
              <p className="text-gray-300">
                Are you sure you want to delete {deleteCount} author{deleteCount !== 1 ? 's' : ''}? This action cannot be undone.
              </p>
              <div className="flex gap-3">
                <button
